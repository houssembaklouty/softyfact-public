...
const pagePrice = computed(() => $page.props.orderAmount ?? 149)
...
const faqs = computed(() => [
  { q: t('faq_offline_q1'), a: t('faq_offline_a1') },
  { q: t('faq_offline_q2'), a: t('faq_offline_a2') },
  { q: t('faq_offline_q3'), a: t('faq_offline_a3') },
  { q: t('faq_offline_q4'), a: t('faq_offline_a4') },
  { q: t('faq_offline_q5'), a: t('faq_offline_a5') },
  { q: t('faq_offline_q6'), a: t('faq_offline_a6') },
])
...
const filteredCities = computed(() => {
  ...
  return q
    ? tunisCities.filter(c => c.toLowerCase().includes(q))
    : tunisCities
})

watch(citySearch, () => { cityHighlightIndex.value = 0 })

const selectCity = (city) => {
  form.value.ville = city
...
const openCityDropdown = () => {
  ...
  citySearch.value = ''
  const idx = tunisCities.indexOf(form.value.ville)
  cityHighlightIndex.value = idx >= 0 ? idx : 0
  nextTick(() => {
...
const scrollHighlightedIntoView = () => {
  nextTick(() => {
    const list = cityListRef.value
    if (!list) return
    const item = list.children[cityHighlightIndex.value]
...
const handleCityKeydown = (e) => {
  ...
  if (e.key === 'ArrowDown') {
    ...
    e.preventDefault()
    cityHighlightIndex.value = Math.max(cityHighlightIndex.value - 1, 0)
    scrollHighlightedIntoView()
  } else if (e.key === 'Enter') {
    e.preventDefault()
    const highlighted = filteredCities.value[cityHighlightIndex.value]
    if (highlighted) {
      selectCity(highlighted)
    } else if (filteredCities.value.length === 1) {
...
const isFormValid = computed(() => {
  const digits = form.value.telephone.replace(/\s/g, '')
  return /^[0-9]{8}$/.test(digits) && form.value.ville
})
...
const submitOrder = async () => {
  validatePhone()
  if (phoneError.value || !form.value.ville) return
...
const jsonLd = computed(() => JSON.stringify({
  ...
  '@type': 'Product',
  name: `${productName.value} — Version Bureau — Logiciel de facturation`,
...
<template>
  ...
  <div class="min-h-screen bg-cm-surface font-sans text-cm-on-surface antialiased">
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 pb-28 lg:pb-12">
      ...
      <div class="text-center mb-6 sm:mb-12">
        ...
        <div class="inline-flex items-center gap-2 bg-cm-primary/10 px-4 py-1.5 rounded-full mb-4">
          ...
          <span class="text-xs font-bold text-cm-primary tracking-widest uppercase">{{ t('offlineBadge') }}</span>
        </div>
        ...
        <div class="mt-3 hidden">
          <Link href="/product/online" class="inline-flex items-center gap-1.5 text-sm text-blue-500 font-medium hover:underline">
        ...
        <div class="order-1 lg:order-1">
          ...
          <!-- <div class="flex items-center justify-center gap-4 mt-4 sm:mt-6">
            <div class="text-center">
              <p class="text-xs sm:text-sm text-cm-outline line-through">500 DT</p>
              <p class="text-3xl sm:text-4xl font-black text-cm-primary">{{ pagePrice }} <span class="text-sm sm:text-base font-bold text-cm-secondary">DT</span></p>
            </div>
            <div class="bg-cm-error text-cm-on-primary text-sm font-bold px-4 py-2 rounded-full animate-pulse">
              -70%
            </div>
          </div> -->
        </div>
        ...
        <div class="order-2 lg:order-2">
          <div id="order-form" class="bg-white rounded-3xl shadow-2xl border border-cm-outline-variant/10 p-4 sm:p-8 sticky top-24">
            ...
            <div v-if="submitted" class="text-center py-8">
              ...
              <p class="text-cm-secondary">{{ t('offlineOrderConfirm') }}</p>
              <Link href="/" class="mt-6 inline-block text-cm-primary hover:underline font-medium">
                {{ t('backToHome') }}
              </Link>
            </div>
            ...
            <div v-else>
              ...
              <h2 class="font-headline font-bold text-xl text-cm-on-background mb-2">
                {{ t('orderDesktop', { name: productName }) }}
              </h2>
              <p class="text-sm text-cm-secondary mb-4 sm:mb-6">
                {{ t('fillInfoOrCall') }} <a :href="'tel:' + ($page.props.supportPhone || '').replace(/\s/g, '')" dir="ltr" class="text-cm-primary font-semibold hover:underline inline-block">{{ $page.props.supportPhone }}</a>
              </p>
              ...
              <form @submit.prevent="submitOrder" class="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    class="mt-1 w-full bg-cm-surface px-4 py-3 rounded-xl border-none ring-1 ring-cm-on-background/30 focus:ring-2 focus:ring-cm-primary transition-all"
                </div>
                ...
                <div v-if="$page.props.showVersionSwitch" class="flex items-center justify-center bg-gray-100 rounded-xl p-1">
                  <span class="flex-1 flex items-center justify-center gap-1.5 px-4 py-2.5 rounded-lg text-sm font-bold bg-cm-primary text-white shadow-sm transition-all">
                    <span class="material-symbols-outlined text-base" style="font-variation-settings: 'FILL' 1;">desktop_windows</span>
                    {{ t('desktopVersion') }}
                  </span>
                  <Link href="/product/online" class="flex-1 flex items-center justify-center gap-1.5 px-4 py-2.5 rounded-lg text-sm font-bold text-gray-500 hover:text-blue-500 transition-all">
                    <span class="material-symbols-outlined text-base">cloud</span>
                    {{ t('cloudVersion') }}
                  </Link>
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    :class="['mt-1 w-full px-4 py-3 rounded-xl bg-cm-surface border-none ring-1 focus:ring-2 transition-all',
                      phoneError ? 'ring-cm-error focus:ring-cm-error' : 'ring-cm-on-background/30 focus:ring-cm-primary']"
                  />
                ...
                <div ref="cityDropdownRef" class="relative">
                  <label class="text-xs font-bold text-cm-on-background uppercase">
                    {{ t('city') }} <span class="text-cm-error">*</span>
                  </label>
                  <button
                    type="button"
                  ...
                  >
                    <span>{{ form.ville || t('cityPlaceholder') }}</span>
                  ...
                  <div
                    ...
                    class="absolute z-50 mt-1 w-full bg-white border border-cm-outline-variant/20 rounded-xl shadow-xl overflow-hidden"
                  >
                    <div class="p-2 border-b border-cm-outline-variant/10">
                      <div class="relative">
                        ...
                        <input
                          ...
                          class="w-full pl-9 pr-3 py-2 text-sm rounded-lg bg-cm-surface border-none ring-1 ring-cm-on-background/30 focus:ring-2 focus:ring-cm-primary outline-none"
                          @click.stop
                          @keydown="handleCityKeydown"
                        />
                      </div>
                    </div>
                    <ul ref="cityListRef" class="max-h-48 overflow-y-auto py-1">
                      <li
                        v-for="(city, index) in filteredCities"
                        :key="city"
                        @click="selectCity(city)"
                        @mouseenter="cityHighlightIndex = index"
                        :class="['px-4 py-2.5 text-sm cursor-pointer transition flex items-center justify-between',
                          cityHighlightIndex === index
                            ? 'bg-cm-primary-container/10 text-cm-primary font-medium'
                            : 'text-cm-on-surface hover:bg-cm-surface-container-low']"
                      >
                        <span>{{ city }}</span>
                        <span v-if="form.ville === city" class="material-symbols-outlined text-cm-primary text-lg">check</span>
                      </li>
                      <li v-if="filteredCities.length === 0" class="px-4 py-3 text-sm text-cm-outline text-center">
                        {{ t('noCityFound') }}
                      </li>
                    </ul>
                  </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    class="mt-1 w-full bg-cm-surface px-4 py-3 rounded-xl border-none ring-1 ring-cm-on-background/30 focus:ring-2 focus:ring-cm-primary transition-all resize-none"
                  ></textarea>
                </div>
                ...
                <button
                  ...
                  class="btn-cta-shine btn-buy w-full py-4 bg-cm-primary hover:scale-[1.02] disabled:opacity-50 disabled:cursor-not-allowed text-cm-on-primary font-bold text-base sm:text-lg rounded-xl shadow-lg shadow-[#006B59]/20 transition-all flex items-center justify-center gap-2"
                >
                  <span v-if="submitting" class="material-symbols-outlined animate-spin text-xl">progress_activity</span>
                  <span v-if="!submitting">{{ t('orderNow') }}</span>
      ...
      <div class="mt-12 sm:mt-20 bg-cm-inverse-surface rounded-3xl p-6 sm:p-12 text-center relative overflow-hidden">
        ...
        <div class="relative z-10">
          <h2 class="font-headline font-extrabold text-2xl sm:text-3xl text-white mb-3">{{ t('readyTransform') }}</h2>
          <p class="text-cm-inverse-on-surface/70 mb-8 max-w-xl mx-auto">
            {{ t('readyTransformSub') }}
          </p>
          <div class="flex flex-col sm:flex-row items-center justify-center gap-4">
            <a href="#" @click.prevent="scrollToForm" class="w-full sm:w-auto px-6 sm:px-8 py-3 bg-white text-cm-primary font-bold rounded-xl hover:scale-[1.02] active:scale-95 transition-all shadow-lg">
              {{ t('orderNowPrice', { price: pagePrice }) }}
            </a>
    ...
    <div v-if="!submitted" class="fixed bottom-0 left-0 right-0 z-50 lg:hidden bg-white/95 backdrop-blur-xl border-t border-cm-outline-variant/20 shadow-2xl px-4 py-3">
      <button
        @click="scrollToForm"
        class="btn-buy w-full py-3 sm:py-4 bg-cm-primary text-cm-on-primary font-bold text-sm sm:text-base rounded-xl shadow-lg shadow-[#006B59]/30 transition-all flex items-center justify-center gap-2 active:scale-95"
      >
        <span>{{ t('commandDesktop') }}</span>
